# import pandas_ta as ta
# import pandas as pd
# from backtesting import Strategy, Backtest

# def supertrend1(high_data,low_data,close_data):
#     o=ta.supertrend(high_data,low_data,close_data,length=10)
#     print(o)
#     return o['SUPERTd_10_3.0']




# class supertrend(Strategy):

#     def init(self):
#         self.super1 = self.I(supertrend1, self.data.High.s, self.data.Low.s, self.data.Close.s)


#     def next(self):
#         # Ensure there are at least 4 data points before accessing
#         if len(self.data.Close) < 4:
#             return  # Skip execution if there aren't enough data points
        
#         if (self.data.Close[-4] > self.data.Open[-4] and 
#             self.data.Close[-3] > self.data.Open[-3] and 
#             self.data.Close[-2] > self.data.Open[-2] and 
#             self.data.Close[-1] > self.data.Open[-1] and 
#             self.super1[-1] == 1):
            
#             if self.position.is_short:
#                 self.position.close()
#             self.buy()

#         elif self.super1[-1] == -1:
#             if self.position.is_long:
#                 self.position.close()
            


# # import yfinance as yf
# # data=yf.download('AMZN',start='2020-05-24',end='2024-06-29',interval='1D')
# # print(data)

# data=pd.read_csv('TATAMOTOR_1Hour.csv')
# data['Date']=pd.to_datetime(data['Date'])
# data['Date']=data['Date'].dt.tz_localize(None)
# data.set_index('Date',inplace=True)
# print(data)

# print(supertrend1(data['High'], data['Low'], data['Close']))


# bt=Backtest(data,supertrend,cash=100000)
# output=bt.run()
# bt.plot()
# print(output)


import pandas_ta as ta
import pandas as pd
from backtesting import Strategy, Backtest

# Function for direction (buy/sell signal)
def supertrend1(high_data, low_data, close_data):
    o = ta.supertrend(high_data, low_data, close_data, length=10)
    return o['SUPERTd_10_3.0']

# Function for Supertrend value (to plot on chart)
def supertrend2(high_data, low_data, close_data):
    o = ta.supertrend(high_data, low_data, close_data, length=10)
    return o['SUPERT_10_3.0']


class supertrend(Strategy):
    def init(self):
        self.super1 = self.I(supertrend1, self.data.High.s, self.data.Low.s, self.data.Close.s)
        self.super2 = self.I(supertrend2, self.data.High.s, self.data.Low.s, self.data.Close.s)  # Required for plotting

    def next(self):
        # Ensure there are at least 4 data points before accessing
        if len(self.data.Close) < 4:
            return
        
        if (self.data.Close[-4] > self.data.Open[-4] and 
            self.data.Close[-3] > self.data.Open[-3] and 
            self.data.Close[-2] > self.data.Open[-2] and 
            self.data.Close[-1] > self.data.Open[-1] and 
            self.super1[-1] == 1):

            if self.position.is_short:
                self.position.close()
            self.buy()

        elif self.super1[-1] == -1:
            if self.position.is_long:
                self.position.close()


# Load your data
data = pd.read_csv('D:\intrative broker\strategy testing\TATAMOTOR_1Hour.csv')
data['Date'] = pd.to_datetime(data['Date'])
data['Date'] = data['Date'].dt.tz_localize(None)
data.set_index('Date', inplace=True)

# Just for checking output
print(data)
print(supertrend1(data['High'], data['Low'], data['Close']))
print(supertrend2(data['High'], data['Low'], data['Close']))

# Backtest
bt = Backtest(data, supertrend, cash=100000)
output = bt.run()
bt.plot()
print(output)
